...
cloud_cb ()
{
  ...
  int number = 0;
  int tmp_frame_code = frame_code;
  tmp_frame_code++;
  while(tmp_frame_code!=0){
	  number++;
	  tmp_frame_code/=10;
  }
  frame_code ++;
  if(number>10)
	  return 0;
  std::string frame_prefix = std::string (10 - number, '0');
  cout<<"number"<<number<<endl;

  std::stringstream intstr;
  intstr<<frame_code;
  std::string frame_postfix = intstr.str();

  std::string file_name = std::string (frame_prefix);
  file_name += frame_postfix;
  file_name += ".pcd";

  std::string dir_name = std::string ("./records/");
...
main (int argc, char** argv)
{
  if (argc < 2)
    {
      PCL_WARN("Please set pcd_filename(e.g. $ %s sample.pcd)\n", argv[0]);
      exit (1);
    }
